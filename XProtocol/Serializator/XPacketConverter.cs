...
namespace XProtocol.Serializator
{
    public class XPacketConverter
    {
        ...
        public static XPacket Serialize(byte type, byte subtype, object obj, bool strict = false)
        {
            ...
            foreach (var field in fields)
            {
                if (field.Item1.FieldType == typeof(string))
                {
                    packet.SetStringValue(field.Item2, field.Item1.GetValue(obj).ToString());
                    continue;
                }
                packet.SetValueType(field.Item2, field.Item1.GetValue(obj));
            }
